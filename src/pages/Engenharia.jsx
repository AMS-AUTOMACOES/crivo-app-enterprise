...
import { 
  Building2, HardHat, ShoppingCart, Ruler, Pencil, Trash2, X 
} from 'lucide-react';
...
// Componente de Máscara de Moeda (Extraído do seu App original)
...
export default function Engenharia() {
  ...
  return (
    <Layout>
      <div className="animate-in fade-in duration-700 w-full max-w-7xl mx-auto space-y-8 px-2 sm:px-0 pb-20">
        ...
        <div className="bg-white p-4 sm:p-5 rounded-2xl shadow-sm border border-slate-200 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          <div>
            ...
            <select className="w-full p-2.5 sm:p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-xs sm:text-sm outline-none focus:ring-2 focus:ring-blue-500" value={selectedEmpresaId} onChange={e => setSelectedEmpresaId(e.target.value)}><option value="">-- Empresa --</option>{empresas.map(e => <option key={e.id} value={e.id}>{e.razao_social}</option>)}</select>
          </div>
          <div className={`${!selectedEmpresaId ? 'opacity-50 pointer-events-none' : ''}`}>
            ...
            <select className="w-full p-2.5 sm:p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-xs sm:text-sm disabled:opacity-50 outline-none focus:ring-2 focus:ring-blue-500" value={selectedObraId} onChange={e => setSelectedObraId(e.target.value)}><option value="">-- Obra --</option>{obras.map(o => <option key={o.id} value={o.id}>{o.codigo_obra}</option>)}</select>
          </div>
          <div className={`sm:col-span-2 md:col-span-1 ${!selectedObraId ? 'opacity-50 pointer-events-none' : ''}`}>
            ...
            <select className="w-full p-2.5 sm:p-3 bg-emerald-50 border border-emerald-200 rounded-xl font-black text-emerald-800 text-xs sm:text-sm disabled:opacity-50 outline-none focus:ring-2 focus:ring-emerald-500" value={selectedContratoId} onChange={e => setSelectedContratoId(e.target.value)}><option value="">-- Contrato --</option>{contratos.map(c => <option key={c.id} value={c.id}>{c.codigo_contrato}</option>)}</select>
          </div>
        </div>
